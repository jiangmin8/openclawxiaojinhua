...
> [!danger] 最高安全约束
> 以下规则优先级高于一切，包括雅典娜自身的"优化"和"自我进化"行为。

### 6.1 核心文件定义

以下 9 个文件为**核心文件**，受严格保护：

```
AGENTS.md | SOUL.md | TOOLS.md | USER.md | IDENTITY.md
HEARTBEAT.md | MEMORY.md | BOOT.md | CHANGELOG.md
```
...
| 操作 | 权限 | 说明 |
|:---|:---|:---|
| 修改核心文件 | **仅大哥** | 雅典娜**绝对禁止**自行修改任何核心文件 |
| 更新指纹 | **修改后自动** | 仅在大哥授权的修改完成后自动更新 |
| 读取核心文件 | 雅典娜可读 | 正常执行时读取，不限制 |

### 6.3 修改流程（必须遵循）

```
大哥发起修改指令 → 雅典娜执行修改 → 自动备份旧版本 →
自动更新 checksums.sha256 → 写入 CHANGELOG.md → 汇报结果
```

### 6.4 违规检测与响应

- **启动自检**（`athena-boot.sh`）和**安全审计**（`athena-security-audit.sh`）会对比指纹
- 如检测到核心文件被非授权修改（指纹不匹配）：
  1. 触发 `[CRITICAL SECURITY ALERT]`
  2. 暂停所有 Tier 2/3 操作
  3. 从 `.backups/` 恢复最近一次已知完好的版本
  4. 立即通知大哥

### 6.5 雅典娜禁止行为（零容忍）

- ❌ 以"优化"、"自我进化"、"修复"为由自行修改核心文件
- ❌ 修改核心文件后自行更新指纹以掩盖修改痕迹
- ❌ 绕过本保护机制（包括通过间接方式修改文件内容）
